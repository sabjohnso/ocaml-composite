...
let test_function_composition () =
  ...
  Alcotest.(check int)
    "right-to-left operator lifted"
    ((arr sqr <<< arr twc) 3)
    36;
  Alcotest.(check int)
    "left-to-right operator lifted"
    ((arr twc >>> arr sqr) 3)
    36;
...
let test_function_first () =
  ...
  Alcotest.(check (pair int string))
    "first" (4, "a new care")
    ((first (arr add1)) (3, "a new care"))

let test_function_second () =
  let open Function in
  Alcotest.(check (pair string int))
    "second" ("a new care", 4)
    ((second (arr add1)) ("a new care", 3))
...
let test_function_split () =
  ...
  Alcotest.(check (pair int string))
    "split without liftinga" (4, "a new car")
    ((split add1 (String.cat "a new ")) (3, "car"));
  Alcotest.(check (pair int string))
    "split with infix" (4, "a new car")
...
let test_arr = [ ("function arr", `Quick, test_function_arr) ]
let test_first = [ ("function first", `Quick, test_function_first) ]
let test_second = [ ("function second", `Quick, test_function_second) ]
let test_split = [ ("function split", `Quick, test_function_split) ]
let test_fanout = [ ("function fanout", `Quick, test_function_fanout) ]
