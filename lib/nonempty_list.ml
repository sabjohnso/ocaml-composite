...
let rec rev_append xs (y, ys) =
  match xs with
  | x1, x2 :: xs -> rev_append (x2, xs) (x1, y :: ys)
  | x, [] -> (x, y :: ys)

let rev (x1, xs) =
  match xs with x2 :: xs -> rev_append (x2, xs) (x1, []) | [] -> (x1, [])
...
module Derived_comonad :
  Functor_sigs.Comonad with type 'a t = 'a nonempty_list =
Functor.Comonad_of_minimal_comonad_extend (struct
  type 'a t = 'a nonempty_list

  let extract (x, _) = x

  let extend f (x, xs) =
    let rec loop (y, ys) = function
...
let tl (_, xs) =
  match xs with x :: xs -> (x, xs) | [] -> raise Call_to_tl_with_one_element
