module Function_arrow_choice :
  Arrow_choice_sigs.Arrow_choice with type ('a, 'b) t = 'a -> 'b =
Arrow_choice.Arrow_choice_of_minimal_arrow_choice_choose_split (struct
  ...
  let choose f g mx =
    let open Either in
    match mx with Left x -> Left (f x) | Right y -> Right (g y)
end)
