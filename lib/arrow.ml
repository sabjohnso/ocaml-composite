...
module Basic_arrow_of_minimal_arrow_first (M : Minimal_arrow_first) = struct
  ...
  let second g =
    arr (fun (x, y) -> (y, x)) >> first g >> arr (fun (x, y) -> (y, x))
...
module Arrow_of_minimal_arrow_split_with_basic_category
    (M : Minimal_arrow_split_with_basic_category) =
struct
  module Derived_arrow__ = Arrow_of_minimal_arrow_split (struct
...
module Arrow_of_minimal_arrow_first_with_basic_category
    (M : Minimal_arrow_first_with_basic_category) =
struct
  module Derived_arrow_ = Arrow_of_minimal_arrow_first (struct
    module Temp = Category.Category_of_basic_category (M)
    include Temp
    include M
  end)
